...
8. Create matching `TASK-NNN` rows in `docs/development/02-task-board.md` with `Product`, `Design`, `API`, `Acceptance`, and `Verification` fields. Set the optional `Risk` field from the expected change surface: `risk:dependencies` for dependency or lockfile changes, `risk:secrets` for environment-variable or secret-handling changes, `risk:containers` for container build/runtime changes, or `none` when none apply. Include every applicable label and never invent an unregistered `risk:*` label.
9. Mark a task `Ready` only when its Product, Design, API, Acceptance, and Verification cells link existing local Markdown sources, its Risk labels are complete and supported, its `A-NNN` ID is mapped in `docs/tests/02-acceptance-matrix.md`, and its Ready contract satisfies `references/implementation-readiness-checklist.md`.
