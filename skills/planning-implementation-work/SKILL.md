...
3. Run `bin/governance design implementation-planning-authoring <target> --json` and follow `authoring_tasks[]`. The payload's `decision_policy` is `do_not_guess_task_scope`; each task lists target `documents`, required `sections`, `required_links`, unresolved `open_decisions` such as `task_scope`, `ready_criteria`, `verification_plan`, and `agent_handoff`, `specialist_skills` including `senior-fullstack`, `ci-cd-pipeline-builder`, and `tech-debt-tracker`, and read-only command steps verify-implementation-planning-authoring and refresh-implementation-planning-authoring.
