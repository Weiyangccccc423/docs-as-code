...
4. Run `bin/governance design api-authoring <target> --json` and follow `authoring_tasks[]`. The payload's `decision_policy` is `do_not_guess_contract_details`; each task lists target `documents`, required `sections`, `required_links`, unresolved `open_decisions`, and read-only command steps verify-api-authoring and refresh-api-authoring.
