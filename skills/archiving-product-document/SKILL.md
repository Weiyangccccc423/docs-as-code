...
7. After manual review, run `bin/governance product mark-ready <target> --reviewed --method manual-reviewed-markdown --check --json`, then run the same command without `--check` when `ok` is true. Use the write command's `next_actions` and each action's `cwd` for the next preflight; do not infer the next phase manually.
