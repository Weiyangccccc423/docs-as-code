...
8. For DOCX/HTML, run `bin/governance env --repair --require-tool pandoc --check --target <target> --json`; for PDF, use the same operation-scoped preflight with `--require-tool pdftotext`. Follow only reviewed repair actions. TXT requires no external converter.
9. Run `bin/governance product convert <target> --check --json`; apply with `bin/governance product convert <target> --json` only when preflight is `ok: true`. Require `conversion-report.json`, bounded no-shell execution evidence for external converters, and `product_conversion_status: pending_review`. Never treat conversion as review.
...
| PDF | use bounded Poppler `pdftotext` extraction, then review | layout, tables, columns, or diagrams are ambiguous |
