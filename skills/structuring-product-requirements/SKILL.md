...
5. Preview and create only PRD-supported chapter scaffolds with `bin/governance scaffold product <target> --chapter <chapter-key> --check --json`, then run the same command without `--check` when the plan is correct; use returned `local_commands` for checks, keep returned `next_actions` for later, and keep `NN-<slug>.md` filenames with unique `NN` prefixes.
