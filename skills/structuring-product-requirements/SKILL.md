...
8. Replace every `governance:scaffold-placeholder` with PRD-derived content before leaving this phase.
9. Link every chapter back to `core/PRD.md`.
10. Link every `NN-*.md` chapter from `core/product-meta.md`.
11. Extract acceptance criteria into a stable `NN-*acceptance*.md` chapter before design derivation. Give each criterion a stable unique `A-NNN` ID.
12. Extract success metrics when present.
13. Add cross-domain terms to glossary with unique `Term`, filled `Meaning`, and a `Source` link to the local Markdown definition.
14. Register ambiguous requirements before deriving design. Use unique `U-NNN` IDs and fill `Domain` and `Description`; use `none`, `-`, `n/a`, `non-blocking`, or `resolved` only for items that do not block downstream work.
