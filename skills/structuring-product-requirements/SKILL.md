...
1. Run the product gate:
   ...
   Stop on `ok: false` and repair by `requirements[].code`. When the write command succeeds, keep the returned `next_actions` and `cwd` for later, but finish this phase's authored product chapters before running the next transition.
