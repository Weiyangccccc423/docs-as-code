...
5. Read `references/design-review-checklist.md` and the task's `design_review_report_contract`. With `senior-backend`, write `.governance/design-review-reports/backend-modules-<WORK-ID>.json` so every required decision has one status, rationale, and evidence entry and no blocking finding remains. Run `design review --track backend-modules --work <WORK-ID> --result approved --reason "<authority-review explanation>" --report .governance/design-review-reports/backend-modules-<WORK-ID>.json --reviewed --check --json` before apply. The review binds report content/hash, repository evidence, current reliability evidence, and the loaded `senior-backend` skill SHA-256; changed evidence reopens the decision set.
