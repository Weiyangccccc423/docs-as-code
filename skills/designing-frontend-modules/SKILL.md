...
3. Run `bin/governance design frontend-authoring <target> --json` and follow `authoring_tasks[]`. The payload's `decision_policy` is `do_not_guess_frontend_behavior`; each task lists target `documents`, required `sections`, `required_links`, unresolved `open_decisions` such as `state_ownership` and `error_actions`, and read-only command steps verify-frontend-authoring and refresh-frontend-authoring.
