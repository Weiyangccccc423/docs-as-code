...
13. Read `references/design-review-checklist.md` and the task's `design_review_report_contract`. With `database-designer`, write `.governance/design-review-reports/data-model-<WORK-ID>.json` with every required decision exactly once, current evidence, and no blocking finding. Then run `design review --track data-model --work <WORK-ID> --result approved --reason "<authority-review explanation>" --report .governance/design-review-reports/data-model-<WORK-ID>.json --reviewed --check --json` before apply. The review binds report content/hash, current schema/migration evidence, and the authority skill.
