...
For a single-session execution contract, run `bin/governance workflow work-package <target> --json` or `make work-package`; target-local `.agents/skills` and `.codex/skills` are scanned automatically, and `--skill-root <path>` adds another root. Start only when `package_available` and `can_start` are true. Load `skill_readiness.resolved_requirements`, stop on missing local or authority skills, read every target-local `work_package.read_order` path, constrain edits to `work_package.write_scope`, follow `next_action`, then run the packet's verify and `refresh_command` before requesting another work package. Source-pack checklist paths resolve under `docs/agent-workflow/workflow-pack/references/` in generated targets. In design derivation, follow `work_stage` in the order authoring, integration, review: `author-design-documents` completes track-owned files before reverse-link repairs, and `record-design-review` clears static `open_decisions` only after source- and authority-bound evidence is persisted.
...
If implementation verification reports `design_review_stale`, run the affected design authoring command as a read-only drift plan, load its primary authority skill, and repeat `design review --check` plus apply for that track/work. Do not roll the phase backward or edit stored hashes directly.
