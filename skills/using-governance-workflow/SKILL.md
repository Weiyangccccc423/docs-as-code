...
For a single-session execution contract, run `bin/governance workflow work-package <target> --json` or `make work-package`; target-local `.agents/skills` and `.codex/skills` are scanned automatically, and `--skill-root <path>` adds another root. Start only when `package_available` and `can_start` are true. Load `skill_readiness.resolved_requirements`, read every target-local `work_package.read_order` path, including `docs/agent-workflow/workflow-pack/references/` entries, constrain edits to `work_package.write_scope`, follow `next_action`, then verify and refresh. In design derivation, follow `work_stage` in authoring, integration, threat-review, machine-review, reliability-review, migration-review, review order: `author-design-documents` completes track files, architecture `run-threat-review` records STRIDE/DREAD evidence, API `run-api-review` records contract evidence, backend `run-reliability-review` records SLO applicability and required tool evidence, data-model `run-migration-review` records compatibility/migration/rollback evidence, and `record-design-review` records authority judgment.

For a new-folder handoff, use consumer bootstrap `--workflow-preset implementation-routing`. Inspect `implementation_run_preview`: continue only when `handoff_ready: true`, `status: ready_to_start`, and `next_action.argv` is the single exact `--apply-start --expect-snapshot` action bound to `snapshot.id`. The preset may advance the workflow into implementation, but it never claims, executes, or closes a task, and it cannot be combined with legacy implementation start/closeout flags. A skipped preview must be routed through its stable `skip_code`, `blocked_by`, and `required_advance_applied` fields before retrying.

When the recorded phase is `implementation`, run `bin/governance implementation plan <target> --json` and `bin/governance implementation run <target> --check --json`, or target-local `make implementation-plan` and `make implementation-run-check`, before editing code. Use the plan to select one actionable Ready or In Progress `TASK-NNN`, then use runner `snapshot.id` and `next_action.argv`. A Ready task must be claimed with a separate `--apply-start --expect-snapshot` invocation that returns `status: implementation_required`; stop there for codebase mapping and edits, never combine claim and execution. After edits, require runner `verification_summary.all_ready: true` before `--execute`; stop on the first failure or any unapproved/manual repair. Use the returned snapshot-guarded `--closeout` action only after `status: closeout_ready`, and require final `status: complete` plus `closeout_applied: true`. Lower-level `implementation start`, `implementation verify`, and `implementation closeout` remain diagnostic primitives. Load `executing-implementation-task` and `verifying-governance-docs` from the workflow pack first, then load authority-routing skills such as `senior-fullstack`, `senior-backend`, `senior-qa`, `senior-security`, or `api-design-reviewer` from the agent environment when the plan requires them.
