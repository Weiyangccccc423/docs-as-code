...
When resuming an initialized target, run `bin/governance workflow plan <target> --json` or target-local `make workflow-plan` to inspect the current phase, `blocked` status, active queue summaries, local/authority skill routing summaries, read-only queue commands, `local_commands`, and `next_actions` before choosing a phase-specific skill.
