...
For every design `authoring_tasks[]` payload, run tasks by `sequence` and follow `execution.primary_skill`, `execution.primary_specialist_skill`, `execution.verify_step`, `execution.refresh_step`, and `execution.stop_condition` before editing, verifying, or refreshing the queue. Use `required_links[].status` for deterministic link repair routing; treat `open_decisions` as separate unresolved design questions.
