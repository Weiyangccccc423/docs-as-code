...
- `docs/tests/02-acceptance-matrix.md` uses `Acceptance`, `Design`, `API`, and `Test` columns, each `Acceptance` row has a unique `A-NNN` ID defined in the referenced product acceptance chapter, every product-defined `A-NNN` is mapped or listed as uncovered, and each row links to matching local source docs
