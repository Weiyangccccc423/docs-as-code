...
For backend reliability verification, run `bin/governance design reliability-review <target> --reviewed --check --json` before write mode. Treat `reliability_review_evidence_missing`, `reliability_review_evidence_invalid`, and `reliability_review_evidence_stale` as implementation blockers. Repair applicability, source references, SLI definitions, target basis, validation plan, error-budget policy, or authority-tool findings first; rerun reliability review and only then rerun `design review --track backend-modules --check`. Never edit generated reports or stored hashes directly.
For data-model migration verification, run `bin/governance design migration-review <target> --reviewed --check --json` before write mode. Treat `migration_review_evidence_missing`, `migration_review_evidence_invalid`, and `migration_review_evidence_stale` as implementation blockers. Repair applicability, schema snapshots, migration specification, compatibility acceptances, source references, or authority-tool findings first; rerun migration review and only then rerun `design review --track data-model --check`. Never edit generated plans, reports, runbooks, issue IDs, or stored hashes directly.
Use `python3 scripts/authority_skills.py --json` as an offline source-pack inventory and `--repair --check` for a no-write plan grounded in `references/authority-skills.lock.json`. Inspect `manifest`, `status_counts`, `provenance_issue_skills`, and `repair_plan`. Never infer a source or install argv for unregistered skills. Use `--strict` for availability and `--strict-provenance` when approved immutable source and digest evidence must stop architecture, API, backend, data-model, security, CI, or implementation work.
Use `python3 scripts/bootstrap_consumer_project.py --target <target> --product <product-doc> --profile <profile> --project-name "<name>" --check --json` from an unpacked workflow pack to preflight consumer setup; the payload includes the no-write `authority_skill_inventory.repair_plan`. Add `--strict-authority-skills` for availability or `--strict-authority-provenance` for source/integrity strictness before environment checks or target writes. Run the same command without `--check` to initialize and verify the target, then continue from returned `local_commands` and `next_actions`.
Use `--auto-repair-env` with consumer bootstrap only when package-manager repair without approval/manual intervention is authorized. Inspect `env_auto_repair`; automatic repair is allowed only after `repair_decision.decision: run_repair_actions` and `repair_execution.can_auto_apply: true`. For bootstrap-level branching, prefer `env_auto_repair.decision`, `status`, `stop_before_workflow`, `can_continue`, `can_auto_apply`, `requires_approval`, `manual_repair_required`, `runnable_action_ids`, `approval_action_ids`, `manual_action_ids`, `next_step`, `final_env_check_ok`, and `final_missing_required` before drilling into nested env check payloads.
...
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply` only when the task explicitly authorizes writing source-backed product chapters. It writes the selected product scaffold, runs `product structure --check --json`, applies `product structure --json` using conservative `product_plan.suggested_mappings[].command_arg` values, returns `product_structure_apply`, and still leaves unsupported chapters in manual authoring queues.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation` only after product structure apply is authorized. It runs a clean `verify --check`, runs `advance design-derivation`, returns `design_derivation` and `make design-plan`, and deliberately does not write design scaffold placeholders.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview` when design scaffold planning is needed without writes. It runs `scaffold design --check --json`, returns `design_scaffold_preview`, and leaves write-mode design placeholders for a separate explicit authoring step.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply` only when writing standard design placeholders is authorized. It returns `design_scaffold_apply` and expected `governance_scaffold_placeholder` blockers; treat the failing post-scaffold verification as the next design authoring queue, not as bootstrap failure.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview` when the task needs read-only design queue routing after scaffold apply. It returns `design_authoring_preview` from target-local commands such as `design architecture-authoring`, `design api-authoring`, and `design backend-authoring`, including skills, blockers, active work, and repair actions without writing design content. Inspect ordered `queue_summaries[]`, aggregate `authoring_summary` counts including `queue_status_counts`, `next_queue_id`, `next_active_work`, and top-level `active_work.queue_id` before opening the selected full queue payload under `queues`.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview` when the task needs read-only implementation routing after design queue inspection. It returns `implementation_readiness_preview` from `verify --check`, `gate implementation`, and `implementation plan` payloads without advancing implementation or claiming a task. Use ordered `blockers[]`, `readiness_summary.blocker_count`, `blocker_codes`, `source_counts`, `next_blocker`, and `next_repair_action` before drilling into the raw payloads.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview --implementation-advance-preview` when the task also needs read-only implementation phase-transition routing. It returns `implementation_advance_preview` from `advance implementation --check` without recording the implementation phase.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview --implementation-advance-preview --implementation-advance-apply` only when the task authorizes recording the implementation phase. It returns `implementation_advance_apply`, records the implementation phase only after preview passes, refreshes `make implementation-plan`, and skips without writing state while advance blockers remain.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview --implementation-start-preview` when the task also needs read-only start routing for the selected `TASK-NNN`. It returns `implementation_start_preview` from `implementation start` without applying task status updates.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview --implementation-start-preview --implementation-start-apply` only when the task authorizes claiming one Ready task. It returns `implementation_start_apply`, applies the safe `In Progress` status update only after preview passes, refreshes `make implementation-plan`, and skips without writing state while start blockers remain.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview --implementation-advance-preview --implementation-advance-apply --implementation-start-preview --implementation-start-apply --implementation-closeout-preview` when the task also needs read-only closeout routing for the active `TASK-NNN`. It returns `implementation_closeout_preview` from `implementation closeout` without applying `Done` status updates.
Use `--advance-product-structuring --product-scaffold-preview --product-structure-preview --product-structure-apply --advance-design-derivation --design-scaffold-preview --design-scaffold-apply --design-authoring-preview --implementation-readiness-preview --implementation-advance-preview --implementation-advance-apply --implementation-start-preview --implementation-start-apply --implementation-closeout-preview --implementation-closeout-apply` only when the task authorizes marking the active task complete. It returns `implementation_closeout_apply`, applies synchronized `Done` status only after closeout evidence passes, refreshes `make implementation-plan`, and skips without writing state while closeout blockers remain.
When implementation-routing payloads skip a downstream preview or apply action, branch on `skip_code`, `blocked_by`, and the relevant prerequisite boolean such as `required_preview_ready`, `required_readiness_ok`, or `required_start_applied` before summarizing the human `skip_reason`.
...
bin/governance design architecture-authoring <target>
bin/governance design api-authoring <target>
bin/governance design backend-authoring <target>
bin/governance design data-model-authoring <target>
bin/governance design ui-interaction-authoring <target>
...
bin/governance implementation start <target> --task TASK-NNN
bin/governance implementation start <target> --task TASK-NNN --apply
bin/governance implementation closeout <target> --task TASK-NNN
bin/governance implementation closeout <target> --task TASK-NNN --apply
...
Use `workflow work-package --json` or `make work-package` to verify the one-unit agent contract. Require a stable `package_id`, matching phase/queue/work IDs, bounded `read_order` and `write_scope`, explicit blockers/open decisions/repair actions, `skill_readiness.resolved_requirements`, `next_action`, and read-only `refresh_command`. Treat `ok: true` as packet construction only; require `can_start: true` before work, and require `status: complete` with `package_available: false` as the terminal implementation state.
Use `workflow resume --json` or `make workflow-resume` to verify resumable routing. Require canonical SHA-256 `snapshot.id`, one `selected_action` at most, a matching `assert_snapshot_command`, and `refresh_command`; reject `status: stale`, stop when `stop_before_action` is true, and require `action_count: 0` for `complete`.
When the recorded phase is `implementation`, use `implementation plan --json` plus `implementation run --check --json`, or target-local `make implementation-plan` plus `make implementation-run-check`, after repairing verification findings. Treat `active_work.status: complete` as terminal and `in_progress` as resumable. For a Ready task, require runner `status: ready_to_start`, then execute the returned snapshot-guarded `--apply-start` action; require `status: implementation_required` and stop for code edits. After edits, require all `verification_preflights[]` ready before `--execute`; a failed command, stale snapshot, unavailable lock, manual repair, unknown repair source, or unapproved reviewed-command repair keeps the task `In Progress`. Before `Done`, require embedded `closeout_preview.closeout_ready: true`, then use the returned snapshot-guarded `--closeout` action and require `status: complete` plus `closeout_applied: true`. Use lower-level `implementation start`, `implementation verify`, and `implementation closeout` payloads to diagnose the runner's embedded requirements; never hand-edit synchronized task-board and roadmap statuses.
...
For every design authoring payload, inspect `authoring_summary` (`document_status_counts`, `non_authored_document_count`, `open_decision_count`, `required_link_status_counts`, `non_satisfied_required_link_count`, and `link_repair_action_count`). Follow work-package `work_stage` in authoring, integration, threat-review, machine-review, reliability-review, review order. Use document blockers before cross-track link repairs; treat `required_decisions` as the stable review scope and clear `open_decisions` only when `next_action.kind: record-design-review` completes through a current `design review --check` plus apply. Read `references/design-review-checklist.md`, load the reported primary authority skill, and require its name/SHA-256 plus source/evidence snapshots in `docs/decisions/design-reviews.json`.
Treat `design_review_invalid`, `design_review_missing`, `design_review_orphan`, and `design_review_stale` as implementation blockers. Repair malformed JSON or repeat the exact track/work review after current evidence is authored and linked; do not suppress stale findings by editing stored hashes. In the implementation phase, design authoring commands and `design review` remain available for this repair. Roadmap/task-board `Status`, task-board `Verification`, and verification-log evidence changes remain allowed; semantic planning changes and PRD, acceptance, architecture, API, backend, data, UI/frontend, or test evidence changes require renewed review.
...
After successful `design architecture-authoring --json`, use `decision_policy: do_not_guess_architecture_boundaries`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive system context, container, and quality-attribute edits. Each task lists architecture `documents`, required `sections`, `required_links`, unresolved `open_decisions` such as `system_boundary`, `container_responsibilities`, `quality_scenarios`, `deployment_assumptions`, and `adr_candidates`, `specialist_skills` including `senior-architect`, `senior-security`, `observability-designer`, and `slo-architect`, authority-routing `missing_policy`, and command steps such as verify-architecture-authoring and refresh-architecture-authoring; run those read-only commands before considering the architecture track repaired.
...
After successful `design backend-authoring --json`, use `decision_policy: do_not_guess_backend_boundaries`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive backend module and external-service edits. Each task lists backend `documents`, required `sections`, `required_links`, unresolved `open_decisions` such as `module_boundaries` and `observability`, `specialist_skills` including `senior-backend`, `observability-designer`, `slo-architect`, and `senior-security`, and command steps such as verify-backend-authoring and refresh-backend-authoring; run those read-only commands before considering the backend track repaired.
After successful `design data-model-authoring --json`, use `decision_policy: do_not_guess_data_model`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive entity, schema, migration, and rollback edits. Each task lists data-model `documents`, required `sections`, `required_links`, unresolved `open_decisions` such as `entity_ownership`, `transaction_boundaries`, `migration_order`, and `rollback_strategy`, `specialist_skills` including `database-designer`, `database-schema-designer`, `migration-architect`, `senior-backend`, and `senior-security`, and command steps such as verify-data-model-authoring and refresh-data-model-authoring; run those read-only commands before considering the data-model track repaired.
After successful `design ui-interaction-authoring --json`, use `decision_policy: do_not_guess_ui_behavior`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive visible flow, screen, state, error, accessibility, and copy edits. Each task lists `authoring_summary`, `sequence`, `execution` metadata with `primary_skill`, `primary_specialist_skill`, `verify_step`, `refresh_step`, and `stop_condition`, UI `documents`, required `sections`, `required_links`, `required_links[].status`, `required_link_status_counts`, `non_satisfied_required_link_count`, `link_repair_actions`, `link_repair_action_count`, `repair_strategy`, `verify_command`, `refresh_command`, authority-routing `missing_policy`, unresolved `open_decisions` such as `primary_flows`, `screens`, `states`, `error_actions`, `accessibility`, and `copy_and_content`, `specialist_skills` including `senior-frontend` and `a11y-audit`, and command steps such as verify-ui-interaction-authoring and refresh-ui-interaction-authoring; run those read-only commands before considering the UI interaction track repaired.
After successful `design frontend-authoring --json`, use `decision_policy: do_not_guess_frontend_behavior`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive frontend module and API-consumption edits. Each task lists `authoring_summary`, `sequence`, `execution` metadata with `primary_skill`, `primary_specialist_skill`, `verify_step`, `refresh_step`, and `stop_condition`, frontend `documents`, required `sections`, `required_links`, `required_links[].status`, `required_link_status_counts`, `non_satisfied_required_link_count`, `link_repair_actions`, `link_repair_action_count`, `repair_strategy`, `verify_command`, `refresh_command`, authority-routing `missing_policy`, unresolved `open_decisions` such as `state_ownership` and `error_actions`, `specialist_skills` including `senior-frontend`, `a11y-audit`, and `performance-profiler`, and command steps such as verify-frontend-authoring and refresh-frontend-authoring; run those read-only commands before considering the frontend track repaired.
After successful `design test-strategy-authoring --json`, use `decision_policy: do_not_guess_verification_scope`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive test strategy and acceptance-matrix edits. Each task lists `authoring_summary`, `sequence`, `execution` metadata with `primary_skill`, `primary_specialist_skill`, `verify_step`, `refresh_step`, and `stop_condition`, test `documents`, required `sections`, `required_links`, `required_links[].status`, `required_link_status_counts`, `non_satisfied_required_link_count`, `link_repair_actions`, `link_repair_action_count`, `repair_strategy`, `verify_command`, `refresh_command`, authority-routing `missing_policy`, unresolved `open_decisions` such as `acceptance_coverage` and `evidence_targets`, `specialist_skills` including `senior-qa`, `playwright-pro`, `a11y-audit`, and `security-pen-testing`, and command steps such as verify-test-strategy-authoring and refresh-test-strategy-authoring; run those read-only commands before considering the verification track repaired.
After successful `design implementation-planning-authoring --json`, use `decision_policy: do_not_guess_task_scope`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive roadmap, task-board, and verification-log edits. Each task lists `authoring_summary`, `sequence`, `execution` metadata with `primary_skill`, `primary_specialist_skill`, `verify_step`, `refresh_step`, and `stop_condition`, development `documents`, required `sections`, `required_links`, `required_links[].status`, `required_link_status_counts`, `non_satisfied_required_link_count`, `link_repair_actions`, `link_repair_action_count`, `repair_strategy`, `verify_command`, `refresh_command`, authority-routing `missing_policy`, unresolved `open_decisions` such as `task_scope`, `ready_criteria`, `verification_plan`, and `agent_handoff`, `specialist_skills` including `senior-fullstack`, `ci-cd-pipeline-builder`, and `tech-debt-tracker`, and command steps such as verify-implementation-planning-authoring and refresh-implementation-planning-authoring; run those read-only commands before considering the delivery-planning track repaired.
After successful `design architecture-decisions-authoring --json`, use `decision_policy: do_not_guess_architecture_decisions`, `skill_requirements`, `authority_skill_requirements`, and `authoring_tasks[]` to drive ADR trigger review and ADR authoring. Each task lists `authoring_summary`, `sequence`, `execution` metadata with `primary_skill`, `primary_specialist_skill`, `verify_step`, `refresh_step`, and `stop_condition`, ADR `documents`, required `sections`, `required_links`, `required_links[].status`, `required_link_status_counts`, `non_satisfied_required_link_count`, `link_repair_actions`, `link_repair_action_count`, `repair_strategy`, `verify_command`, `refresh_command`, authority-routing `missing_policy`, unresolved `open_decisions` such as `adr_trigger`, `decision_scope`, and `alternatives`, keeps `requires_adr: undetermined` until sources prove the trigger, returns `specialist_skills` including `senior-architect`, `migration-architect`, and `tech-stack-evaluator`, and includes command steps such as verify-architecture-decisions-authoring and refresh-architecture-decisions-authoring; run those read-only commands before considering the ADR track repaired.
...
Treat `product_chapter_disposition_invalid` and `product_chapter_disposition_stale` as product-decision blockers: repair malformed records or rerun reviewed `product disposition --check` and apply against the current canonical PRD hash before trusting an omission.
...
- at least one implementation task is `Ready` before starting or `In Progress` while being executed
