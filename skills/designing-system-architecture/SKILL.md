...
6. Only after threat evidence is current, read `references/design-review-checklist.md` and the task's `design_review_report_contract`. With `senior-architect`, write `.governance/design-review-reports/architecture-<WORK-ID>.json` so every required decision has one status, rationale, and evidence entry and no blocking finding remains. Run `design review --track architecture --work <WORK-ID> --result approved --reason "<authority-review explanation>" --report .governance/design-review-reports/architecture-<WORK-ID>.json --reviewed --check --json` before apply. The Git-tracked review binds the report content/hash, PRD, acceptance, architecture and threat evidence, and the loaded `senior-architect` skill SHA-256.
