...
Check mode only probes `uv` and returns the temporary-write plan. Full mode defaults to offline `uv` operation, builds exactly one wheel, installs it into a disposable Python 3.10+ environment, verifies `dac`, its compatibility alias and help, proves `dac init --check` leaves the fresh target unchanged, initializes the target, and verifies installed plus generated-target continuation commands. It also proves installed commands discover the governed root from a nested directory, accept `-C` after the command, and that target-local `bin/dac` stays bound to its generated root. Missing `uv` returns `install_smoke_uv_unavailable` with a no-write manual repair route.

The project root must contain exactly one supported product document unless `dac init <product-document>` selects one explicitly. Use `dac -C <project> ...` or `dac COMMAND -C <project> ...` to select another working directory. Existing-project commands search the selected directory and its parents for the nearest root containing both the target runtime and generated runtime manifest. This stable identity does not depend on mutable workflow state, so a missing or damaged state file can still be diagnosed from a nested directory. Initialization remains bound to the exact selected directory. Normal CLI operation must remain local after installation; initialization, verification, status, continuation routing, and runtime refresh must not require network access.

Human mode invokes the same JSON contracts internally and renders bounded summaries instead of unbounded path or evidence lists. `--json` remains the lossless automation interface. Parse failures print a command-specific help recovery path, and long options reject prefix abbreviations so adding a future option cannot reinterpret an existing invocation. Before any `dac init` write, the CLI runs a read-only bootstrap preflight and requires product selection `explicit` or `auto-discovered`; `none` and `ambiguous` stop with `writes_state: false`. The selected absolute product path is pinned into the apply command, and JSON apply output records the read-only selection under `cli_preflight`.
...
The target-local wrapper exposes `status`, `next`, `verify`, `doctor`, and `help`, provides examples for every command, and resolves its target from the generated runtime location rather than the caller's current working directory. It therefore remains bound to the generated project when called from a nested directory. `init` and `upgrade` remain source-pack operations: run them from the workflow pack or an installed `dac` command so initialization and runtime refresh use the source pack's trusted implementation.
...
Use `runtime refresh --check --json` before repair when an agent needs a no-write preflight. It reports `would_refresh`, `would_remove`, `version_transition`, and `migration_plan` while leaving target files and `.governance/state.json` unchanged. The plan identifies managed runtime paths, `preserved_project_document_roots`, exact ordered argv steps, `requires_trusted_artifact` rollback evidence, a deterministic `plan_id`, and aggregate `source_identity` and `target_identity` digests. `source_identity.artifact_verification` records `pack-manifest.json` presence, verification status, manifest SHA-256, and deterministic finding codes. Same-version, compatible-upgrade, and clean legacy-install refreshes may use a source checkout without that manifest. Breaking upgrades, rollbacks, version replacements, and conflicting or invalid evidence require both an exported artifact with `verified: true` and the exact reviewed `--approve-version-transition --expect-migration-plan <plan_id>` write command. Failed artifact verification sets `blocked_by: trusted-artifact-verification`; approval, a valid plan ID, or check-mode success cannot waive it. Missing or stale plan identity performs no writes.
