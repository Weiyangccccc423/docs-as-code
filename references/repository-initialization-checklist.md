...
- If pip installation was unavailable, did the short offline artifact entry `./docs-as-code-workflow-pack/bin/dac init --check` pass before `./docs-as-code-workflow-pack/bin/dac init`? Use `governance-bootstrap` only when Git initialization or advanced workflow presets must be composed in one invocation.
