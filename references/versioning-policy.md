...
Use target `runtime refresh --check --json` from a trusted source workflow-pack checkout before applying an upgrade. Inspect `version_transition.from_version`, `to_version`, `classification`, `evidence_status`, `candidate_versions`, `approval_required`, and `can_apply`, then review the returned `migration_plan` `plan_id`, `source_identity`, `target_identity`, steps, and scope. The deterministic classifications are `same`, `compatible_upgrade`, `breaking_upgrade`, `rollback`, `version_replacement`, and `legacy_install`; SemVer precedence ignores build metadata, so changing only build metadata is a `version_replacement` rather than an upgrade.

Write mode may apply `same`, `compatible_upgrade`, and clean `legacy_install` transitions from a source checkout without extra approval or an export manifest. A breaking upgrade, rollback, build-metadata replacement, invalid version evidence, or disagreement between snapshot `VERSION` and state `workflow_pack_version` requires a reviewed check result, an exported artifact whose `pack-manifest.json` verification has `verified: true`, and `--approve-version-transition --expect-migration-plan <plan_id>`. `source_identity.artifact_verification` records manifest presence, verification status, manifest SHA-256, and finding codes. The plan ID binds this evidence, the source file inventory, target governance state and manifests, transition evidence, target path, and managed scope. A missing or mismatched ID or failed artifact verification blocks before changing any target file; approval cannot waive either failure. Rerun check whenever the source or target changes. Approval does not waive post-refresh verification or migration work required by a Major release.
...
Rollback uses a previously verified artifact and the same check-then-apply procedure, followed by the explicit version-transition approval. The migration plan marks `requires_trusted_artifact: true` for rollback and every other high-risk transition; apply is enabled only when `source_identity.artifact_verification.verified` is true. Do not reconstruct an earlier version by editing generated manifests or state. Re-run target-local verification after either upgrade or rollback.
