...
Write mode may apply `same`, `compatible_upgrade`, and clean `legacy_install` transitions without extra approval. A breaking upgrade, rollback, build-metadata replacement, invalid version evidence, or disagreement between snapshot `VERSION` and state `workflow_pack_version` requires a reviewed check result plus `--approve-version-transition --expect-migration-plan <plan_id>`. The plan ID binds the source file inventory, target governance state and manifests, transition evidence, target path, and managed scope. A missing or mismatched ID fails before changing any target file; rerun check whenever the source or target changes. Approval does not waive post-refresh verification or migration work required by a Major release.
