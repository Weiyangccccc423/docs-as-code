...
- Run `python3 scripts/smoke_workflow_pack_artifact.py --archive dist/docs-as-code-workflow-pack.tar.gz --json` to unpack the exact release tar.gz artifact and run manifest verification, `verify_pack`, fresh-target initialization, target-local command checks, consumer bootstrap with `--auto-repair-env --workflow-preset product-structure` and `--auto-repair-env --workflow-preset design-scaffold`, plus dry-run checks from the unpacked artifact.
