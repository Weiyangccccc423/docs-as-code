...
- Are `next_actions` sorted by `sequence`, paired with `preflight_for` and `requires_action`, and gated on `success_condition: ok:true` before any apply action runs?
...
- Does the plan expose `version_transition`, and are breaking upgrades, rollbacks, replacements, and conflicting/invalid evidence blocked until reviewed `--approve-version-transition` is explicit?
