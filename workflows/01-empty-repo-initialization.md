...
3. Initialize the target folder:
   ...
   When `--json` is used, the success payload includes `local_commands` with target-local `make` entries plus `next_actions` with the next preflight/apply workflow commands. Run each action's `argv` from its reported `cwd`. Follow `next_actions` instead of assuming product structuring is immediately available.
