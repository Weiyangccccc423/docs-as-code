...
The bootstrap script runs `verify_pack_manifest`, `verify_pack`, non-strict `authority_skill_inventory`, `env --repair --check`, `init --check`, write-mode `init`, then target-local `bin/governance verify . --check --json`, `make governance-status`, `make workflow-plan`, and `make work-package`. Its success payload includes `authority_skill_inventory`, `strict_authority_skills`, `work_package`, `local_commands`, and `next_actions` so agents can continue from the generated target without rerunning `status`. Use `--strict-authority-skills` when missing authority-routing skills must fail before environment checks or target writes.
