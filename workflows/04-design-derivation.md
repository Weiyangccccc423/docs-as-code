...
2. Create the standard design document scaffold when files are missing:
   ...
   Prefer `workflow work-package --json` or `make work-package` for repeated agent sessions. It scans every track in stable order and performs seven passes: `work_stage: authoring` returns `next_action.kind: author-design-documents` for the first task whose own `documents[].status` is not `authored` or `reference_template`; `work_stage: integration` selects the first remaining non-satisfied cross-track link; architecture-only `work_stage: threat-review` returns `next_action.kind: run-threat-review`; API-only `work_stage: machine-review` returns `next_action.kind: run-api-review`; backend-only `work_stage: reliability-review` returns `next_action.kind: run-reliability-review`; data-model-only `work_stage: migration-review` returns `next_action.kind: run-migration-review`; `work_stage: review` selects the first unreviewed or stale authority decision set. The package returns one stable `work_id`, target-local `read_order`, constrained `write_scope`, and verify/refresh contract. This prevents reverse links from blocking owning documents and prevents architecture, API, backend, or data-model authority signoff before deterministic checks.

   When `next_action.kind` is `run-api-review`, require `docs/api/openapi.json`, load `api-design-reviewer`, and run its command contract with `--reviewed --min-grade B --check`, then apply. The governance runtime invokes `api_linter.py`, `breaking_change_detector.py`, and `api_scorecard.py` directly with the current Python interpreter. It accepts zero lint errors and warnings, no breaking or potentially breaking changes, and grade B or better; the first pass creates `docs/api/baselines/openapi-baseline.json`. Reports and hash-bound evidence are written under `docs/api/reviews/` only after every tool and policy check passes.

   When `next_action.kind` is `run-reliability-review`, derive `docs/backend/reliability/slo-scope.json` from product acceptance, `docs/architecture/03-quality-attributes.md`, `docs/backend/01-modules.md`, and `docs/backend/03-external-services.md`. Load `slo-architect` and decide applicability before choosing numbers. `required` mode needs at least one source-backed SLO and `docs/backend/04-error-budget-policy.md`; provisional prelaunch targets need a concrete validation plan. `not-applicable` mode must contain no SLOs but still needs an owner, source-backed reason, and revisit triggers. Run `design reliability-review --reviewed --check --json`, inspect the output, then apply. Required mode invokes `slo_designer.py`, `error_budget_calculator.py`, and `slo_review.py`; every finding fails closed. Evidence records the explicit `target_percent` to reviewer `target` adapter, source hashes, skill/tool hashes, and generated report hashes.

   When `next_action.kind` is `run-migration-review`, derive the five authored inputs under `docs/backend/migrations/` from product acceptance, architecture quality attributes, backend ownership, and `docs/backend/02-data-model.md`. Load `database-schema-designer` for schema judgment and `migration-architect` for machine evidence. Run `design migration-review --reviewed --check --json`, inspect stable compatibility issue IDs, and add a decision to `compatibility-acceptances.json` only when a breaking or potentially-breaking issue has a named owner, concrete reason, mitigation, and repository evidence. Then rerun preflight and apply. The command invokes `migration_planner.py`, `compatibility_checker.py`, and `rollback_generator.py` on isolated copies, removes nondeterministic timestamps, and binds every input/source/skill/tool/report hash. `accepted_with_mitigations` is valid only after all serious issues are explicitly accepted; orphan acceptances fail closed.

   When `next_action.kind` is `record-design-review`, read `references/design-review-checklist.md`, load the reported `authority_skill`, and consume the returned `report_contract`. Use that authority skill to write one structured report directly under `.governance/design-review-reports/`; cover every `required_decision_ids` item exactly once with status, rationale, and current repository evidence, and resolve every blocking finding. Pass `--report <path>` to the returned command contract first with `--check`, then without it. The command records `docs/decisions/design-reviews.json` with the full report, report SHA-256, current canonical PRD, acceptance source, repository evidence, and authority `SKILL.md` SHA-256. Every track/acceptance pair must cover the exact `required_decisions`; `open_decisions` clears only while the record is current. Report, source, or design evidence changes make the record stale and block implementation. `not-applicable` is limited to ADR-trigger review; an approved ADR review requires `--evidence docs/decisions/NNN-<slug>.md`. Roadmap/task-board `Status`, task-board `Verification`, and verification-log evidence changes are allowed after implementation begins; semantic hashes still detect task scope, traceability, and other planning changes.
   ...
   Use `design api-candidates --json` and `design api-authoring --json` for API work. API candidates include `acceptance_id`, `suggested_endpoint_file`, `replaceable_starter_endpoint`, and open decisions before a contract is authored. API authoring includes `docs/api/openapi.json` as a structured document, uses `decision_policy: do_not_guess_contract_details`, and retains `verify-api-authoring` plus `refresh-api-authoring`; invalid JSON, unsupported OpenAPI versions, missing info, or empty paths remain authoring blockers. After links are satisfied, `design api-review` produces the machine evidence required by `design review --track api-contracts`.
   ...
   Use `design backend-authoring --json` for backend module and external-service work. It uses `decision_policy: do_not_guess_backend_boundaries`, `open_decisions` such as `module_boundaries` and `observability`, `specialist_skills` such as `senior-backend`, `observability-designer`, `slo-architect`, and `senior-security`, and command steps `verify-backend-authoring` and `refresh-backend-authoring`.
...
7. Read `references/backend-design-checklist.md`, `references/data-model-design-checklist.md`, and `references/backend-operability-checklist.md`, then create or complete `docs/backend/` implementation design:
   ...
   Decide SLO applicability before backend authority signoff. Start from `templates/docs/backend/reliability/slo-scope.json`; use `required` only when product or architecture sources justify a measurable production objective, and use reviewed `not-applicable` evidence otherwise. For required mode, complete `docs/backend/04-error-budget-policy.md`, run `design reliability-review --reviewed --check`, inspect all three authority-tool outputs, then apply.
8. Create database and lifecycle design:
   ...
   - `references/data-model-design-checklist.md` checklist coverage
   Before data-model authority signoff, start from `templates/docs/backend/migrations/`, author the applicability scope and required schema/migration inputs, and run `design migration-review --reviewed --check`. Inspect compatibility and rollback outputs before apply; never treat a nonzero compatibility result as approved without matching written acceptance evidence.
...
16. After an accepted architecture or ADR source selects the implementation stack, load `configuring-project-runtime`, `tech-stack-evaluator`, `senior-architect`, `senior-devops` when repair execution is in scope, and the owning implementation specialist. Run `bin/governance project-env plan <target> --json` and inspect `required_commands`, `command_coverage`, `missing_command_registrations`, `tool_readiness`, `coverage_status`, and `configuration_complete`. The design workflow exposes this as the final `project-runtime` queue and returns a `project-runtime-configuration` work package after design authoring and reviews complete. Register each explicit executable, safe version probe, numeric version constraint, reviewed source, local Markdown evidence, and manual or exact reviewed-command repair policy with `project-env register --reviewed --check --json` before apply. Do not use `--replace` without reviewing the existing and proposed contracts. Registration does not install tools and must stop when any field would be guessed. For an unavailable reviewed-command tool, run `project-env repair --tool-id <tool-id> --check --json`; inspect source, argv, cwd, write scope, timeout, and output bounds before requesting approval for apply. Do not continue until `configuration_complete: true`, implementation requirement `project_runtime_ready` passes, post-repair readiness passes, and `.governance/project-environment-repairs.json` has no pending record.
...
- Every current `A-NNN` and design track has a non-stale authority review in `docs/decisions/design-reviews.json`, including a current `.governance/design-review-reports/*.json` content/hash binding with exact decision coverage and no blocking findings; malformed, missing, orphaned, or stale records block implementation.
- The final `project-runtime` queue is `complete`, `configuration_complete` is true, and implementation gate requirement `project_runtime_ready` passes. `command_contract_invalid`, `repair_evidence_pending`, `registration_required`, and `repair_required` remain blockers.
...
- `docs/backend/reliability/review-evidence.json` is valid and current; `reliability_review_evidence_missing`, `reliability_review_evidence_invalid`, and `reliability_review_evidence_stale` block backend signoff and implementation.
- `docs/backend/migrations/review-evidence.json` is valid and current; `migration_review_evidence_missing`, `migration_review_evidence_invalid`, and `migration_review_evidence_stale` block data-model signoff and implementation.
