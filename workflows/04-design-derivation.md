...
2. Create the standard design document scaffold when files are missing:
   ...
   Use `design frontend-authoring --json` for UI/frontend work. It uses `decision_policy: do_not_guess_frontend_behavior`, `open_decisions` such as `state_ownership` and `error_actions`, `specialist_skills` such as `senior-frontend`, `a11y-audit`, and `performance-profiler`, and command steps `verify-frontend-authoring` and `refresh-frontend-authoring`.
