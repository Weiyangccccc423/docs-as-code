...
```
   ...
   For resumed work, run `workflow-resume`, assert `snapshot.id` through `assert_snapshot_command.argv`, execute exactly one `selected_action`, and then run `refresh_command.argv`. The installable `dac next --apply --json` command may perform this sequence only after its target-bound command and approval checks pass. Discard any action that returns `status: stale`; stop on `blocked`, `approval_required`, or `failed` instead of reconstructing a command from memory.
...
5. If verification reports target-local runtime or workflow-pack snapshot drift, including invalid or mismatched `VERSION`, manifest `pack_version`, or state `workflow_pack_version`, inspect the refresh plan from a trusted source workflow-pack checkout before writing repairs:
   ...
   bin/governance runtime refresh <target> --approve-version-transition --expect-migration-plan <plan_id> --json
   ```

   Treat the `--check` form as the no-write repair plan. Inspect `version_transition.from_version`, `to_version`, `classification`, `evidence_status`, `candidate_versions`, `approval_required`, and `can_apply`, then execute only enabled `migration_plan.steps` in sequence. Review `plan_id`, aggregate `source_identity`, `source_identity.artifact_verification`, aggregate `target_identity`, and scope together; any source or target drift invalidates the approval. The plan's `preserved_project_document_roots` are never refresh outputs. For a high-risk plan with `requires_trusted_artifact: true`, require `artifact_verification.present: true`, `verified: true`, a SHA-256 `manifest_sha256`, and empty `finding_codes`; otherwise apply remains disabled with `blocked_by: trusted-artifact-verification`. Apply only a reviewed source version under `references/versioning-policy.md`. Use normal write mode from a source checkout for `same`, `compatible_upgrade`, or clean `legacy_install`; for a `breaking_upgrade`, `rollback`, `version_replacement`, or conflicting/invalid version repair, use an exported, manifest-verified artifact and the reviewed `--approve-version-transition --expect-migration-plan <plan_id>` command. Untrusted, unapproved, unbound, or stale-plan high-risk write mode must fail before changing files. Successful write mode records the source version in the snapshot, both manifests, and state. Then use returned `local_commands[].argv` for target-local checks and `next_actions[].argv` for the next workflow transition.
