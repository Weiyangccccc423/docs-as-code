...
5. If verification reports target-local runtime or workflow-pack snapshot drift, including invalid or mismatched `VERSION`, manifest `pack_version`, or state `workflow_pack_version`, inspect the refresh plan from a trusted source workflow-pack checkout before writing repairs:
   ...
   Treat the `--check` form as the no-write repair plan. Inspect `version_transition.from_version`, `to_version`, `classification`, `evidence_status`, `candidate_versions`, `approval_required`, and `can_apply`, then execute only enabled `migration_plan.steps` in sequence. The plan's `preserved_project_document_roots` are never refresh outputs; a high-risk plan's `requires_trusted_artifact` must be satisfied before approval. Apply only a reviewed source version under `references/versioning-policy.md`. Use normal write mode only for `same`, `compatible_upgrade`, or clean `legacy_install`; use `--approve-version-transition` only after reviewing a `breaking_upgrade`, `rollback`, `version_replacement`, or conflicting/invalid version evidence. Unapproved high-risk write mode must fail before changing files. Successful write mode records the source version in the snapshot, both manifests, and state. Then use returned `local_commands[].argv` for target-local checks and `next_actions[].argv` for the next workflow transition.
