...
Source-pack release, dry-run, and artifact-smoke orchestrators must bound every child step: release steps use a 60-minute outer limit and dry-run/artifact steps use 15-minute limits. Treat `timed_out: true` as a failed gate and inspect the captured step `argv`, `stdout`, and `stderr` before retrying.
