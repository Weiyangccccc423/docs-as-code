...
3. If the target project has a Makefile, run its verification entry:
   ...
   `governance-status` runs `bin/governance status . --json`; on success, its payload includes `local_commands` so resumed agents can rediscover the target-local `make` command contract without re-running initialization. It also includes `next_actions` with `cwd`, workflow paths, required skills, preflight commands, and apply commands for the next state transition.
