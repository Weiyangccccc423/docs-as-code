...
For each project check, add one structured command-contract row, run `bin/governance implementation verify <target> --task TASK-NNN --command command-name --check --json`, then execute the returned command. Inspect `environment_readiness` before execution: bare executables are resolved through the effective `PATH`, relative executables are resolved from contract `Cwd` and must remain inside the repository, and explicit absolute paths must exist and be executable. A missing tool already registered by `check_env.py` routes to the no-write governance environment repair preflight; an unknown project tool stops for approved source and install-policy registration without guessing a package or command. The runner uses exact `Argv`/`Cwd` without a shell, refuses approval-required rows, requires `--allow-writes` for state-writing rows, bounds runtime and output, preserves every run in `docs/development/04-implementation-evidence.md`, and keeps one current verification-log row per `(Task, Command)`. Closeout requires `evidence_summary.all_verification_results_passing: true`.
