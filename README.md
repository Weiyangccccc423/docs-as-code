...
Use `workflow work-package --json` to execute design in deterministic passes. `work_stage: authoring` and `author-design-documents` complete track-owned documents; `work_stage: integration` repairs cross-track links; architecture `work_stage: threat-review` and `next_action.kind: run-threat-review` run `senior-security` STRIDE/DREAD review; API `work_stage: machine-review` and `next_action.kind: run-api-review` run `api-design-reviewer`; backend `work_stage: reliability-review` and `next_action.kind: run-reliability-review` run the SLO applicability and `slo-architect` gate; `work_stage: review` and `record-design-review` record authority judgment. Author `docs/architecture/threat-model/scope.json` and `mitigations.json` from the bundled templates, require complete DFD/STRIDE coverage, and give every DREAD >= 7 threat a named owner, concrete mitigation, and repository evidence before architecture signoff. API review still requires zero lint errors/warnings, no breaking or potentially breaking changes, and grade B or better. For backend reliability, author `docs/backend/reliability/slo-scope.json`; choose `required` with source-backed SLOs or `not-applicable` with an owner, reason, sources, and revisit triggers. Reviews persist in `docs/decisions/design-reviews.json` with authority `SKILL.md` SHA-256 provenance. Missing, invalid, or stale threat/API/reliability evidence or design reviews block implementation.
