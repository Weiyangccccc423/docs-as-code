...
After an accepted architecture or ADR source selects the stack, load `configuring-project-runtime`, `tech-stack-evaluator`, and `senior-architect`, then run `bin/governance project-env plan <target> --json`. Register each explicit tool with `project-env register --reviewed --check --json`; inspect the preview, then repeat without `--check`. Use `--replace` only for a reviewed change to an existing tool ID. The command never installs software or guesses a package, version range, source, or repair instruction.
