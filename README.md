...
Use `workflow plan --json` at any initialized phase to get the current phase, read-only queue commands, `blocked` status, compact active queue summaries, local/authority skill routing summaries, ordered `skill_loading_plan` steps, `local_commands`, and `next_actions` without manually probing each phase-specific command.
