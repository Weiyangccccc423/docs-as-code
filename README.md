...
The source workflow pack also carries `.github/workflows/ci.yml`, which pins Python 3.10 and Node 22, then runs the CI baseline on GitHub: `make test`, `make stack-acceptance`, `python3 scripts/verify_pack.py --json`, and `python3 scripts/check_env.py --json`. Its `workflow_dispatch` trigger allows an explicitly re-enabled workflow to rerun the current ref without creating an empty commit; it does not enable a workflow that a repository administrator disabled.
