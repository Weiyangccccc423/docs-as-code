...
Use `--workflow-preset` for the standard fast path: `product-structure` expands through product structuring apply, `design-scaffold` expands through design scaffold apply, `design-routing` adds read-only design authoring queue routing, and `implementation-routing` advances only when design is ready and then returns a read-only guarded-runner handoff. It never claims, executes, or closes an implementation task, and the CLI rejects combining this runner handoff with legacy implementation start/closeout flags. Presets only set existing bootstrap flags; normal dependency checks, write gates, skip behavior, and `workflow_preset_expanded_flags` in the JSON payload make the expanded actions auditable.
