...
Use `workflow work-package --json` to execute design in three deterministic passes. `work_stage: authoring` and `next_action.kind: author-design-documents` complete only the selected track's own declared documents; `work_stage: integration` repairs cross-track `required_links` after their owning documents exist; `work_stage: review` and `next_action.kind: record-design-review` require the primary authority skill before resolving the static decision set. Preflight and apply the returned contract with `bin/governance design review <target> --track <track> --work <WORK-ID> --result approved --reason "<authority-review explanation>" --reviewed --check --json`, then without `--check`. Reviews are stored in `docs/decisions/design-reviews.json` with PRD, acceptance, evidence, and authority `SKILL.md` SHA-256 provenance. Missing, malformed, orphaned, or stale reviews block implementation. During implementation, roadmap/task-board `Status`, task-board `Verification`, and verification-log evidence updates remain valid; changing task scope, traceability, or other reviewed planning meaning makes the review stale. Design authoring and `design review` commands remain available in the implementation phase to inspect and renew stale evidence without rolling workflow state backward.
