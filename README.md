...
When `--auto-repair-env` is supplied, the bootstrap script may run write-mode `env --repair --json` and then re-run `env --repair --check --json`, but only when the initial repair preflight reports `repair_decision.decision: run_repair_actions`, no approval actions, no manual repair actions, and `repair_execution.can_auto_apply: true`. The returned `env_auto_repair` payload records whether repair was applied or skipped and preserves the final env check used before initialization.
