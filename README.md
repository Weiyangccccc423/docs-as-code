...
TXT sources are converted automatically into reviewable Markdown with `docs/product/core/source/conversion-report.json` hash evidence. DOCX and HTML first elevate only Pandoc through `--require-tool pandoc`, then use the same bounded no-shell conversion path. The initialized repository remains `product_conversion_status: pending_review` and workflow resume returns a guarded `product-mark-ready` handoff; conversion never counts as semantic approval. PDF stays archived and blocked for reviewed manual extraction.
