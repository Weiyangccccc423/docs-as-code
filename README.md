...
Use `workflow work-package --json` to execute design in four deterministic passes. `work_stage: authoring` and `next_action.kind: author-design-documents` complete only the selected track's own declared documents; `work_stage: integration` repairs cross-track `required_links`; API `work_stage: machine-review` and `next_action.kind: run-api-review` run the three `api-design-reviewer` tools against `docs/api/openapi.json`; `work_stage: review` and `next_action.kind: record-design-review` require the primary authority skill. API machine review requires zero lint errors/warnings, no breaking or potentially breaking changes, and scorecard grade B or better before authority signoff. Reviews are stored in `docs/decisions/design-reviews.json` with PRD, acceptance, evidence, and authority `SKILL.md` SHA-256 provenance. Missing, invalid, orphaned, or stale API evidence or design reviews block implementation; renew machine evidence before the API design review when contract inputs change.
