...
class GovernanceScriptsTest(unittest.TestCase):
    ...
    def test_verify_reports_governance_state_updated_at_before_last_verification(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            state["last_verification"] = {
                ...
                "warnings": [],
                "findings": [],
                "checked_at": "2026-01-01T00:01:00+00:00",
            }
            state_path.write_text(json.dumps(state, indent=2, sort_keys=True), encoding="utf-8")

            report = verify(root)

            message = "governance state updated_at must not be older than last_verification.checked_at"
            self.assertIn(message, report.errors)
            self.assertIn(
                {
                    "code": "state_timestamp_updated_at_stale",
                    "severity": "error",
                    "path": ".governance/state.json",
                    "message": message,
                },
    ...
    def test_verify_reports_governance_state_last_verification_summary_entry_invalid(self) -> None:
        cases = [
            ("errors", [123], "governance state last_verification errors entries must be strings"),
            ("warnings", [123], "governance state last_verification warnings entries must be strings"),
        ]
        for key, value, message in cases:
            with self.subTest(key=key):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    state["last_verification"][key] = value
                    state_path.write_text(json.dumps(state, indent=2, sort_keys=True), encoding="utf-8")
                    ...
                    self.assertIn(
                        {
                            "code": "state_last_verification_field_invalid",
                            "severity": "error",
                            "path": ".governance/state.json",
                            "message": message,
                        },
    ...
    def test_verify_reports_governance_state_last_verification_ok_error_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            message = "governance state last_verification ok must be false when errors are present"
            self.assertIn(message, report.errors)
            self.assertIn(
                {
    ...
    def test_verify_reports_governance_state_last_verification_ok_finding_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            message = "governance state last_verification ok must be false when error findings are present"
            self.assertIn(message, report.errors)
            self.assertIn(
                {
                    "code": "state_last_verification_ok_mismatch",
    ...
    def test_verify_reports_governance_state_last_verification_finding_invalid(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                {
                    "code": "state_last_verification_finding_invalid",
                    "severity": "error",
                    "path": ".governance/state.json",
                    "message": "governance state last_verification findings entries must be objects",
                },
                [finding.to_dict() for finding in report.findings],
            )

    def test_verify_reports_governance_state_last_verification_finding_field_invalid(self) -> None:
        cases = [
            (
                {"code": "example", "severity": "error", "path": ".governance/state.json"},
                "governance state last_verification findings entries must include string message",
            ),
            (
                {
                    "code": "",
    ...
    def test_verify_reports_governance_phase_history_invalid_advanced_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            state_path = root / ".governance/state.json"
            state = json.loads(state_path.read_text(encoding="utf-8"))
            state["phase_history"][0]["advanced_at"] = "not-a-timestamp"
            state_path.write_text(json.dumps(state, indent=2, sort_keys=True), encoding="utf-8")

            report = verify(root)

            self.assertIn(
                "governance state phase_history advanced_at for phase product-structuring "
                "must be an ISO timestamp with timezone",
                report.errors,
            )
            self.assertIn(
                {
                    "code": "state_phase_history_advanced_at_invalid",
                    "severity": "error",
                    "path": ".governance/state.json",
                    "message": (
                        "governance state phase_history advanced_at for phase product-structuring "
    ...
    def test_verify_reports_stale_governance_state_last_gate_while_initialized(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            message = "governance state last_gate must be absent while phase is initialized"
            self.assertIn(message, report.errors)
            self.assertIn(
                {
                    "code": "state_phase_last_gate_stale",
                    "severity": "error",
                    "path": ".governance/state.json",
                    "message": message,
                },
    ...
    def test_verify_reports_governance_state_last_gate_checked_at_invalid(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            state_path = root / ".governance/state.json"
            state = json.loads(state_path.read_text(encoding="utf-8"))
            state["last_gate"]["checked_at"] = "not-a-timestamp"
            state_path.write_text(json.dumps(state, indent=2, sort_keys=True), encoding="utf-8")

            report = verify(root)

            self.assertIn(
                "governance state last_gate checked_at for phase product-structuring "
                "must be an ISO timestamp with timezone",
                report.errors,
            )
            self.assertIn(
                {
                    "code": "state_phase_last_gate_checked_at_invalid",
                    "severity": "error",
                    "path": ".governance/state.json",
                    "message": (
                        "governance state last_gate checked_at for phase product-structuring "
    ...
    def test_verify_reports_target_governance_status_makefile_recipe_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                "Makefile target governance-status must run command: bin/governance status . --json",
                report.errors,
            )
            self.assertIn(
                {
                    ...
                    "severity": "error",
                    "path": "Makefile",
                    "message": "Makefile target governance-status must run command: bin/governance status . --json",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_reports_target_repair_env_check_makefile_recipe_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "Makefile target repair-env-check must run command: "
                "bin/governance env --repair --check --target . --json",
                report.errors,
            )
            self.assertIn(
    ...
    def test_verify_reports_task_handoff_verification_record_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "docs/agent-workflow/task-handoff.md Verification Record section must preserve guardrail: evidence",
                report.errors,
            )
            self.assertIn(
                {
                    ...
                    "severity": "error",
                    "path": "docs/agent-workflow/task-handoff.md",
                    "message": "docs/agent-workflow/task-handoff.md Verification Record section must preserve guardrail: evidence",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_rejects_invalid_archived_product_source_hash_format(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_archive_hash_invalid",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: archive.sha256 must be a lowercase SHA-256 hex digest",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_rejects_invalid_source_hash_format(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_source_hash_invalid",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: source.sha256 must be a lowercase SHA-256 hex digest",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_rejects_invalid_product_import_reviewed_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_import_reviewed_at_invalid",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: import.reviewed_at must be an ISO timestamp with timezone",
                },
                [finding.to_dict() for finding in report.findings],
            )

    def test_verify_rejects_ready_reviewed_import_without_reviewed_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_import_reviewed_at_missing",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "product import status ready_for_structuring with reviewed conversion requires import.reviewed_at",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_rejects_source_filename_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest_path = root / "docs/product/core/source/source-manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["source"]["filename"] = "other.md"
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True),
                encoding="utf-8",
            )
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_source_filename_mismatch",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: source.filename must match archive.path filename",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_rejects_backslash_archived_product_source_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
    ...
    def test_verify_rejects_product_source_not_provided_with_archive(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_source_provided_invalid",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: source.provided must be true when product source is archived",
                },
                [finding.to_dict() for finding in report.findings],
            )

    def test_verify_rejects_missing_product_source_original_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_source_original_path_invalid",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: source.original_path must be a non-empty string",
                },
                [finding.to_dict() for finding in report.findings],
            )

    def test_verify_rejects_product_source_original_path_filename_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            manifest_path = root / "docs/product/core/source/source-manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["source"]["original_path"] = str(root / "other.md")
            manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_source_original_path_mismatch",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: source.original_path filename must match source.filename",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_verify_rejects_missing_product_import_conversion_method(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.md"
            product.write_text("# Demo\n", encoding="utf-8")
            ...
            self.assertIn(
                {
                    "code": "product_source_manifest_import_conversion_method_invalid",
                    "severity": "error",
                    "path": "docs/product/core/source/source-manifest.json",
                    "message": "invalid product source manifest: import.conversion_method must be a non-empty string",
                },
                [finding.to_dict() for finding in report.findings],
            )
    ...
    def test_bootstrap_installs_target_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_start = status_result.stdout.find("{")
            self.assertNotEqual(-1, payload_start, status_result.stdout)
            status_payload, _ = json.JSONDecoder().raw_decode(status_result.stdout[payload_start:])
            ...
            self.assertEqual(0, repair_check_result.returncode, repair_check_result.stderr)
            payload_start = repair_check_result.stdout.find("{")
            self.assertNotEqual(-1, payload_start, repair_check_result.stdout)
            repair_check_payload, _ = json.JSONDecoder().raw_decode(repair_check_result.stdout[payload_start:])
            self.assertTrue(repair_check_payload["ok"])
    ...
    def test_product_mark_ready_rejects_invalid_product_import_reviewed_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_inconsistent_ready_import(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_missing_conversion_method(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_missing_source_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn("invalid product source manifest: missing source object", result.errors)
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertEqual("conversion_required", manifest["import"]["status"])

    def test_product_mark_ready_rejects_source_not_provided(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_missing_source_original_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_source_original_path_filename_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
            ...
            manifest_path = root / "docs/product/core/source/source-manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["source"]["original_path"] = str(root / "other.docx")
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True),
                encoding="utf-8",
            )
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertEqual("conversion_required", manifest["import"]["status"])
            self.assertEqual(str(root / "other.docx"), manifest["source"]["original_path"])

    def test_product_mark_ready_rejects_source_size_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
            ...
            manifest_path = root / "docs/product/core/source/source-manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["source"]["size_bytes"] = manifest["archive"]["size_bytes"] + 1
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True),
                encoding="utf-8",
            )
    ...
    def test_product_mark_ready_rejects_source_hash_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_invalid_archive_hash_format(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
    ...
    def test_product_mark_ready_rejects_invalid_source_hash_format(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertEqual("conversion_required", manifest["import"]["status"])
            self.assertEqual("NOT-A-SHA256", manifest["source"]["sha256"])

    def test_product_mark_ready_rejects_source_filename_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            product = root / "product.docx"
            product.write_bytes(b"fake docx bytes")
            ...
            manifest_path = root / "docs/product/core/source/source-manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["source"]["filename"] = "other.docx"
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True),
                encoding="utf-8",
            )
    ...
    def test_check_env_main_repair_check_text_reports_manual_repairs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "target"
            target.mkdir()
            statuses = [
            ...
            original_detect_package_manager = check_env_module.detect_package_manager
            original_collect_git_status = check_env_module.collect_git_status
            sys.argv = ["check_env.py", "--strict", "--repair", "--check", "--target", str(target)]
            check_env_module.collect_status = lambda: statuses
            check_env_module.collect_system_status = lambda: system
            check_env_module.detect_package_manager = lambda _system=None: package_manager
            check_env_module.collect_git_status = lambda _target: git
