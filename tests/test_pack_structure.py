...
class PackStructureTest(unittest.TestCase):
    ...
    def test_verify_pack_reports_incomplete_fresh_target_workflow_smoke_test(self) -> None:
        cases = (
            ("repair-env-check", "repair-env-preview"),
            ("make_product_plan", "source_product_plan"),
            ("make_design_plan", "source_design_plan"),
        )
        for required_phrase, replacement in cases:
            with self.subTest(required_phrase=required_phrase):
                with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_verify_pack_reports_automatic_online_ci_trigger(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_ci_workflow_automatic_trigger"
                    and finding.path == ".github/workflows/ci.yml"
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_consumer_bootstrap_missing_product_structuring_advance(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--advance-product-structuring" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_product_scaffold_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--product-scaffold-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_product_structure_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--product-structure-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_product_structure_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--product-structure-apply" in finding.message
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_consumer_bootstrap_missing_design_scaffold_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--design-scaffold-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_design_scaffold_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--design-scaffold-apply" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_design_authoring_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--design-authoring-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_implementation_readiness_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--implementation-readiness-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_implementation_start_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--implementation-start-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_implementation_start_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--implementation-start-apply" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_implementation_closeout_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--implementation-closeout-preview" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_implementation_closeout_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--implementation-closeout-apply" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_implementation_advance_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--implementation-advance-preview" in finding.message
                    for finding in report.findings
                )
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_consumer_bootstrap_incomplete"
                    and finding.path == "scripts/bootstrap_consumer_project.py"
                    and "--workflow-preset" in finding.message
                    for finding in report.findings
                )
            )

    def test_verify_pack_reports_consumer_bootstrap_missing_auto_repair_env(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
    ...
    def test_verify_pack_reports_release_readiness_missing_artifact_design_routing_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            script.write_text(
                script.read_text(encoding="utf-8").replace(
                    ...
                    "_artifact_smoke_routing_preview_ok",
                ),
                encoding="utf-8",
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_release_readiness_incomplete"
                    and finding.path == "scripts/release_readiness.py"
                    and "source_pack_export_check" in finding.message
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_missing_dry_run_golden_makefile_target(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_makefile_target_missing"
                    and finding.path == "Makefile"
                    and "dry-run-golden" in finding.message
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_missing_artifact_consumer_quick_start_phrase(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            readme.write_text(
                readme.read_text(encoding="utf-8").replace(
                    ...
                    "source commands from the release bundle",
                    1,
                ),
                encoding="utf-8",
            )
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_artifact_consumer_quick_start_missing"
                    and finding.path == "README.md"
                    and "source-pack commands from the unpacked workflow pack" in finding.message
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_missing_product_disposition_source(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            (target / "scripts/product_dispositions.py").unlink()
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_product_disposition_source_missing"
                    and finding.path == "scripts/product_dispositions.py"
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_missing_product_evidence_repair_doc_phrase(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_product_structure_doc_missing"
                    and finding.path == "skills/structuring-product-requirements/SKILL.md"
                    and "evidence_repair_actions" in finding.message
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_missing_ui_interaction_authoring_doc_phrase(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
            ...
            skill = target / "skills/designing-ui-interactions/SKILL.md"
            skill.write_text(
                skill.read_text(encoding="utf-8").replace(
            ...
            self.assertTrue(
                any(
                    finding.code == "pack_ui_interaction_authoring_doc_missing"
                    and finding.path == "skills/designing-ui-interactions/SKILL.md"
                    and "design ui-interaction-authoring" in finding.message
                    for finding in report.findings
                )
            )
    ...
    def test_verify_pack_reports_missing_frontend_authoring_authority_requirement_phrase(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "pack"
            shutil.copytree(
                ROOT,
