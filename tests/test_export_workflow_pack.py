...
class ExportWorkflowPackTest(unittest.TestCase):
    def test_export_creates_manifest_verified_pack_and_archive(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((output / "bin/dac").is_file())
            self.assertTrue((output / "bin/dac").stat().st_mode & 0o111)
            ...
            self.assertTrue((output / "references/authority-skills.lock.json").is_file())
            self.assertTrue((output / "scripts/api_review_evidence.py").is_file())
            self.assertTrue((output / "scripts/threat_review_evidence.py").is_file())
            self.assertTrue((output / "scripts/reliability_review_evidence.py").is_file())
            ...
            self.assertIn("CHANGELOG.md", manifest_paths)
            ...
            self.assertIn("bin/dac", manifest_paths)
            self.assertIn("bin/governance-bootstrap", manifest_paths)
            self.assertIn("references/authority-skills.lock.json", manifest_paths)
            self.assertIn("scripts/api_review_evidence.py", manifest_paths)
            self.assertIn("scripts/threat_review_evidence.py", manifest_paths)
            ...
            self.assertIn("docs-as-code-workflow-pack/CHANGELOG.md", names)
